# Script de Deploy Automatizado - xcafe API
...
Write-Host "🚀 Iniciando deploy da xcafe API SIMPLES..." -ForegroundColor Green
...
# Configurar para versão híbrida (usuário paga)
...
# Usar versão híbrida
Copy-Item "package-extended.json" "package.json"  # Dependências para compilação
Copy-Item "server-hybrid.js" "server.js"         # Servidor que só compila
...
if ($gitStatus) {
    ...
    # Commit
    $commitMessage = "Deploy: $ApiType API - $(Get-Date -Format 'yyyy-MM-dd HH:mm')"
...
Write-Host "Consulte: api/DEPLOYMENT_GUIDE.md" -ForegroundColor White
